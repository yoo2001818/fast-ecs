...
function assertRedBlackInvariant<K, V>(node: Node<K, V> | null): number {
...
describe('RedBlackWoStackSortedMap', () => {
  describe('#set', () => {
    it('should insert nodes', () => {
      const map = new RedBlackSortedMap<number, number>((a, b) => a - b);
      map.set(1, 1);
      map.set(3, 3);
      map.set(2, 2);
    ...
    it('should override value if conflicts', () => {
      const map = new RedBlackSortedMap<number, string>((a, b) => a - b);
      map.set(1, 'a');
      map.set(1, 'b');
  ...
  describe('#delete', () => {
    ...
    it('should correctly remove nodes from complex tree', () => {
      const map = new RedBlackSortedMap<number, number>((a, b) => a - b);
      const sortedInput = Array.from({ length: 10 }, (_, i) => i);
      const input = sortedInput.slice();
      input.forEach((v) => map.set(v, v));
      for (let i = 0; i < input.length - 1; i += 1) {
        const j = (Math.random() * (input.length - 1 - i) | 0) + i;
        const prev = input[i];
        input[i] = input[j];
        input[j] = prev;
      }
